...
# PowerShell script to launch the Feeder service with proper environment setup
...
# Set log file path
$LogFile = Join-Path $LogsDir "feeder.log"
$Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

Write-Host "Starting Feeder service..." -ForegroundColor Yellow
Write-Host "Log file: $LogFile" -ForegroundColor Cyan
Write-Host "Timestamp: $Timestamp" -ForegroundColor Cyan

# Build command arguments
$Args = @("-m", "coin_quant.feeder.service")
...
try {
    ...
    if ($ExitCode -eq 0) {
        Write-Host "✅ Feeder service completed successfully" -ForegroundColor Green
    } else {
        Write-Host "❌ Feeder service exited with code: $ExitCode" -ForegroundColor Red
    }
    
    exit $ExitCode
    
} catch {
    Write-Host "❌ Failed to start Feeder service: $_" -ForegroundColor Red
    exit 1
}
