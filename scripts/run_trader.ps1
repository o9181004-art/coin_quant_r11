...
# PowerShell script to launch the Trader service with proper environment setup
...
Write-Host "Starting Trader service..." -ForegroundColor Yellow
Write-Host "Log file: $LogFile" -ForegroundColor Cyan
Write-Host "Timestamp: $Timestamp" -ForegroundColor Cyan

# Build command arguments
$Args = @("-m", "coin_quant.trader.service")
...
try {
    ...
    if ($ExitCode -eq 0) {
        Write-Host "✅ Trader service completed successfully" -ForegroundColor Green
    } else {
        Write-Host "❌ Trader service exited with code: $ExitCode" -ForegroundColor Red
    }
    
    exit $ExitCode
    
} catch {
    Write-Host "❌ Failed to start Trader service: $_" -ForegroundColor Red
    exit 1
}
